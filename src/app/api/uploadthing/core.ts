...
//https://handlevideo-etwuhgsnbq-uc.a.run.app
    const responseCF = await fetch(endpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json' // Set content type to JSON
      },
      body: JSON.stringify({createdFile, mimeType: extension}) // Stringify the createdFile object
    })
    if(responseCF.ok){
...
export const ourFileRouter: FileRouter = {
  freePlanUploader: f({ pdf: { maxFileSize: "4MB" }, image: {maxFileSize: "4MB"}, video: {maxFileSize: "16GB"}, audio: {maxFileSize: "16GB"} })
